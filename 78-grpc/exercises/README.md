...
**gRPC** — protocols, clients, servers.

## Exercise 1: Core Implementation

Implement and understand the core **gRPC** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./78-grpc/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./78-grpc/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./78-grpc/exercises/solutions/
```
...
```bash
go test ./78-grpc/exercises/solutions/ -run TestInterviewChallenge
```

...
**OAuth2** — auth, crypto, secure coding.

## Exercise 1: Core Implementation

Implement and understand the core **OAuth2** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./97-oauth2/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./97-oauth2/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./97-oauth2/exercises/solutions/
```
...
```bash
go test ./97-oauth2/exercises/solutions/ -run TestInterviewChallenge
```

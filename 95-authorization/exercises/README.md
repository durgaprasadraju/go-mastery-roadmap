...
**Authorization** — auth, crypto, secure coding.

## Exercise 1: Core Implementation

Implement and understand the core **Authorization** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./95-authorization/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./95-authorization/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./95-authorization/exercises/solutions/
```
...
```bash
go test ./95-authorization/exercises/solutions/ -run TestInterviewChallenge
```

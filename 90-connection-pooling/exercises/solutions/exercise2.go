...
// Exercise2Service demonstrates production patterns for Connection Pooling.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Connection Pooling processing cancelled")
	default:
	}
	if data == "" {

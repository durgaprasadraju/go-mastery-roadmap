...
**Helm** — containers, K8s, CI/CD, IaC.

## Exercise 1: Core Implementation

Implement and understand the core **Helm** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./127-helm/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./127-helm/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./127-helm/exercises/solutions/
```
...
```bash
go test ./127-helm/exercises/solutions/ -run TestInterviewChallenge
```

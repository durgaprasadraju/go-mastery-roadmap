...
Implement and understand the core **Load Balancing** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./82-load-balancing/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./82-load-balancing/exercises/solutions/ -run TestService
```
...
```bash
go test ./82-load-balancing/exercises/solutions/ -run TestInterviewChallenge
```

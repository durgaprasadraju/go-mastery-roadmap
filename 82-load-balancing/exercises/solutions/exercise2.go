...
// Exercise2Service demonstrates production patterns for Load Balancing.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Load Balancing processing cancelled")
	default:
	}
	if data == "" {

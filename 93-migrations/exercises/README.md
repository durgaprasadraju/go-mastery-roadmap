# Database Migrations — Exercises

**Database Migrations** — SQL, pooling, transactions.

## Exercise 1: Core Implementation

Implement and understand the core **Database Migrations** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./93-migrations/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./93-migrations/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./93-migrations/exercises/solutions/
```
...
```bash
go test ./93-migrations/exercises/solutions/ -run TestInterviewChallenge
```

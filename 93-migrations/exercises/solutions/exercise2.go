...
// Exercise2Service demonstrates production patterns for Database Migrations.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Database Migrations processing cancelled")
	default:
	}
	if data == "" {

...
**Transactions** — SQL, pooling, transactions.

## Exercise 1: Core Implementation

Implement and understand the core **Transactions** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./87-transactions/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./87-transactions/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./87-transactions/exercises/solutions/
```
...
```bash
go test ./87-transactions/exercises/solutions/ -run TestInterviewChallenge
```

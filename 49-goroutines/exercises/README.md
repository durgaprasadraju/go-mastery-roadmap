...
**Goroutines** concurrency patterns in Go.

## Exercise 1: Core Implementation

Implement and understand the core **Goroutines** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./49-goroutines/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./49-goroutines/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./49-goroutines/exercises/solutions/
```

## Exercise 4: Interview Challenge

**Problem:** Parallel sum with goroutines + WaitGroup

Work in interview-challenge.go for 30 minutes before checking solutions/interview-challenge-solution.go.

```bash
go test ./49-goroutines/exercises/solutions/ -run TestInterviewChallenge
```

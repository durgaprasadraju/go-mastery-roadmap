# Concurrency Patterns — Exercises

**Concurrency Patterns** — goroutines, channels, sync primitives.

## Exercise 1: Core Implementation

Implement and understand the core **Concurrency Patterns** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./64-concurrency-patterns/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./64-concurrency-patterns/exercises/solutions/ -run TestService
```
...
```bash
go test ./64-concurrency-patterns/exercises/solutions/ -run TestInterviewChallenge
```

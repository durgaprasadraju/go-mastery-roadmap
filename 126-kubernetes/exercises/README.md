...
Implement and understand the core **Kubernetes** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./126-kubernetes/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./126-kubernetes/exercises/solutions/ -run TestService
```
...
```bash
go test ./126-kubernetes/exercises/solutions/ -run TestInterviewChallenge
```

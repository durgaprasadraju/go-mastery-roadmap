...
**REST API** — protocols, clients, servers.

## Exercise 1: Core Implementation

Implement and understand the core **REST API** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./77-rest-api/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./77-rest-api/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./77-rest-api/exercises/solutions/
```
...
```bash
go test ./77-rest-api/exercises/solutions/ -run TestInterviewChallenge
```

...
**HTTP** — protocols, clients, servers.

## Exercise 1: Core Implementation

Implement and understand the core **HTTP** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./74-http/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./74-http/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./74-http/exercises/solutions/
```
...
```bash
go test ./74-http/exercises/solutions/ -run TestInterviewChallenge
```

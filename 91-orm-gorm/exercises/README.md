...
**GORM ORM** — SQL, pooling, transactions.

## Exercise 1: Core Implementation

Implement and understand the core **GORM ORM** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./91-orm-gorm/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./91-orm-gorm/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./91-orm-gorm/exercises/solutions/
```
...
```bash
go test ./91-orm-gorm/exercises/solutions/ -run TestInterviewChallenge
```

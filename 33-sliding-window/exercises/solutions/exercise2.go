...
// Exercise2Service demonstrates production patterns for Sliding Window.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Sliding Window processing cancelled")
	default:
	}
	if data == "" {

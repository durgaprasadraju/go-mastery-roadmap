...
**Mutex** concurrency patterns in Go.

## Exercise 1: Core Implementation

Implement and understand the core **Mutex** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./57-mutex/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./57-mutex/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./57-mutex/exercises/solutions/
```

## Exercise 4: Interview Challenge

**Problem:** Thread-safe counter with Mutex

Work in interview-challenge.go for 30 minutes before checking solutions/interview-challenge-solution.go.

```bash
go test ./57-mutex/exercises/solutions/ -run TestInterviewChallenge
```

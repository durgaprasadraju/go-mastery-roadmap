...
Implement and understand the core **Grafana** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./122-grafana/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./122-grafana/exercises/solutions/ -run TestService
```
...
```bash
go test ./122-grafana/exercises/solutions/ -run TestInterviewChallenge
```

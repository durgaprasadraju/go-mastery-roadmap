...
**Profiling** — unit, integration, fuzz, benchmarks.

## Exercise 1: Core Implementation

Implement and understand the core **Profiling** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./110-profiling/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./110-profiling/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./110-profiling/exercises/solutions/
```
...
```bash
go test ./110-profiling/exercises/solutions/ -run TestInterviewChallenge
```

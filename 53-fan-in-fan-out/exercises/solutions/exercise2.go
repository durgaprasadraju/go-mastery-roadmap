...
// Exercise2Service demonstrates production patterns for Fan-In / Fan-Out.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Fan-In / Fan-Out processing cancelled")
	default:
	}
	if data == "" {

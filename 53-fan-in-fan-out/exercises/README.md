# Fan-In / Fan-Out — Exercises

**Fan-In / Fan-Out** — goroutines, channels, sync primitives.

## Exercise 1: Core Implementation

Implement and understand the core **Fan-In / Fan-Out** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./53-fan-in-fan-out/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./53-fan-in-fan-out/exercises/solutions/ -run TestService
```
...
```bash
go test ./53-fan-in-fan-out/exercises/solutions/ -run TestInterviewChallenge
```

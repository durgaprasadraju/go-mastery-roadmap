...
**CI/CD** — containers, K8s, CI/CD, IaC.

## Exercise 1: Core Implementation

Implement and understand the core **CI/CD** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./128-ci-cd/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./128-ci-cd/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./128-ci-cd/exercises/solutions/
```
...
```bash
go test ./128-ci-cd/exercises/solutions/ -run TestInterviewChallenge
```

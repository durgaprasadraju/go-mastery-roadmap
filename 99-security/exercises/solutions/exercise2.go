...
// Exercise2Service demonstrates production patterns for Security Fundamentals.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Security Fundamentals processing cancelled")
	default:
	}
	if data == "" {

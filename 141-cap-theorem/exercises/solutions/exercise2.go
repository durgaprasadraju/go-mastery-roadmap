...
// Exercise2Service demonstrates production patterns for CAP Theorem.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "CAP Theorem processing cancelled")
	default:
	}
	if data == "" {

...
Implement and understand the core **sync.Cond** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./59-cond/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./59-cond/exercises/solutions/ -run TestService
```
...
```bash
go test ./59-cond/exercises/solutions/ -run TestInterviewChallenge
```

...
**Indexing** — SQL, pooling, transactions.

## Exercise 1: Core Implementation

Implement and understand the core **Indexing** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./88-indexing/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./88-indexing/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./88-indexing/exercises/solutions/
```
...
```bash
go test ./88-indexing/exercises/solutions/ -run TestInterviewChallenge
```

...
// Exercise2Service demonstrates production patterns for Math Algorithms.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Math Algorithms processing cancelled")
	default:
	}
	if data == "" {

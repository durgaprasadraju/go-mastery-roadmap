...
// Exercise2Service demonstrates production patterns for Binary Search Trees.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Binary Search Trees processing cancelled")
	default:
	}
	if data == "" {

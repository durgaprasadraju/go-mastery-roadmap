...
Implement and understand the core **Deadlocks** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./63-deadlocks/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./63-deadlocks/exercises/solutions/ -run TestService
```
...
```bash
go test ./63-deadlocks/exercises/solutions/ -run TestInterviewChallenge
```

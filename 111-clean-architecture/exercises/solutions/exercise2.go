...
// Exercise2Service demonstrates production patterns for Clean Architecture.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Clean Architecture processing cancelled")
	default:
	}
	if data == "" {

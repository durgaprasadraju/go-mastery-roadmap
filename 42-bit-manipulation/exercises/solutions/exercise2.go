...
// Exercise2Service demonstrates production patterns for Bit Manipulation.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Bit Manipulation processing cancelled")
	default:
	}
	if data == "" {

...
**Logging** — logs, metrics, traces.

## Exercise 1: Core Implementation

Implement and understand the core **Logging** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./119-logging/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./119-logging/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./119-logging/exercises/solutions/
```
...
```bash
go test ./119-logging/exercises/solutions/ -run TestInterviewChallenge
```

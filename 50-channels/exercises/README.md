...
**Channels** concurrency patterns in Go.

## Exercise 1: Core Implementation

Implement and understand the core **Channels** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./50-channels/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./50-channels/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./50-channels/exercises/solutions/
```

## Exercise 4: Interview Challenge

**Problem:** Producer-consumer with buffered channel

Work in interview-challenge.go for 30 minutes before checking solutions/interview-challenge-solution.go.

```bash
go test ./50-channels/exercises/solutions/ -run TestInterviewChallenge
```

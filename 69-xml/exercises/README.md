...
**XML** — file I/O and serialization.

## Exercise 1: Core Implementation

Implement and understand the core **XML** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./69-xml/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./69-xml/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./69-xml/exercises/solutions/
```
...
```bash
go test ./69-xml/exercises/solutions/ -run TestInterviewChallenge
```

...
**UDP** — protocols, clients, servers.

## Exercise 1: Core Implementation

Implement and understand the core **UDP** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./73-udp/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./73-udp/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./73-udp/exercises/solutions/
```
...
```bash
go test ./73-udp/exercises/solutions/ -run TestInterviewChallenge
```

...
**Query Optimization** — SQL, pooling, transactions.

## Exercise 1: Core Implementation

Implement and understand the core **Query Optimization** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./89-query-optimization/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./89-query-optimization/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./89-query-optimization/exercises/solutions/
```
...
```bash
go test ./89-query-optimization/exercises/solutions/ -run TestInterviewChallenge
```

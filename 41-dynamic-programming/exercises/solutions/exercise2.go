...
// Exercise2Service demonstrates production patterns for Dynamic Programming.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Dynamic Programming processing cancelled")
	default:
	}
	if data == "" {

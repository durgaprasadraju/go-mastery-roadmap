...
**YAML** — file I/O and serialization.

## Exercise 1: Core Implementation

Implement and understand the core **YAML** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./67-yaml/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./67-yaml/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./67-yaml/exercises/solutions/
```
...
```bash
go test ./67-yaml/exercises/solutions/ -run TestInterviewChallenge
```

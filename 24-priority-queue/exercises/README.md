...
Implement **Priority Queues** from scratch with tests and complexity analysis.

## Exercise 1: Core Implementation

Implement and understand the core **Priority Queues** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./24-priority-queue/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./24-priority-queue/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./24-priority-queue/exercises/solutions/
```
...
```bash
go test ./24-priority-queue/exercises/solutions/ -run TestInterviewChallenge
```

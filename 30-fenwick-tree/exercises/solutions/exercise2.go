...
// Exercise2Service demonstrates production patterns for Fenwick Trees.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Fenwick Trees processing cancelled")
	default:
	}
	if data == "" {

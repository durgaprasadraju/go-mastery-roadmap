...
**SQLite** — SQL, pooling, transactions.

## Exercise 1: Core Implementation

Implement and understand the core **SQLite** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./86-sqlite/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./86-sqlite/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./86-sqlite/exercises/solutions/
```
...
```bash
go test ./86-sqlite/exercises/solutions/ -run TestInterviewChallenge
```

...
// Exercise2Service demonstrates production patterns for Circuit Breaker.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Circuit Breaker processing cancelled")
	default:
	}
	if data == "" {

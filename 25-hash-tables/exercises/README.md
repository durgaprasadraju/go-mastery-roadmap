...
Implement **Hash Tables** from scratch with tests and complexity analysis.

## Exercise 1: Core Implementation

Implement and understand the core **Hash Tables** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./25-hash-tables/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./25-hash-tables/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./25-hash-tables/exercises/solutions/
```
...
```bash
go test ./25-hash-tables/exercises/solutions/ -run TestInterviewChallenge
```

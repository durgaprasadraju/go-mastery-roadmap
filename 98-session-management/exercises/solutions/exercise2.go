...
// Exercise2Service demonstrates production patterns for Session Management.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Session Management processing cancelled")
	default:
	}
	if data == "" {

# Session Management — Exercises

**Session Management** — auth, crypto, secure coding.

## Exercise 1: Core Implementation

Implement and understand the core **Session Management** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./98-session-management/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./98-session-management/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./98-session-management/exercises/solutions/
```
...
```bash
go test ./98-session-management/exercises/solutions/ -run TestInterviewChallenge
```

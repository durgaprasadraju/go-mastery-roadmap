...
// Exercise2Service demonstrates production patterns for Atomic Operations.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Atomic Operations processing cancelled")
	default:
	}
	if data == "" {

...
**Testing Fundamentals** — unit, integration, fuzz, benchmarks.

## Exercise 1: Core Implementation

Implement and understand the core **Testing Fundamentals** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./103-testing/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./103-testing/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./103-testing/exercises/solutions/
```
...
```bash
go test ./103-testing/exercises/solutions/ -run TestInterviewChallenge
```

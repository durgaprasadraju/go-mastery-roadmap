...
**E2E Testing** — unit, integration, fuzz, benchmarks.

## Exercise 1: Core Implementation

Implement and understand the core **E2E Testing** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./106-e2e-testing/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./106-e2e-testing/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./106-e2e-testing/exercises/solutions/
```
...
```bash
go test ./106-e2e-testing/exercises/solutions/ -run TestInterviewChallenge
```

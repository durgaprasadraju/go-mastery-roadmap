...
// Exercise2Service demonstrates production patterns for Monotonic Stack.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Monotonic Stack processing cancelled")
	default:
	}
	if data == "" {

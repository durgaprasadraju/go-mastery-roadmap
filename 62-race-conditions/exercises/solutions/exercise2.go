...
// Exercise2Service demonstrates production patterns for Race Conditions.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Race Conditions processing cancelled")
	default:
	}
	if data == "" {

...
Implement **Trees** from scratch with tests and complexity analysis.

## Exercise 1: Core Implementation

Implement and understand the core **Trees** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./21-trees/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./21-trees/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./21-trees/exercises/solutions/
```
...
```bash
go test ./21-trees/exercises/solutions/ -run TestInterviewChallenge
```

...
// Exercise2Service demonstrates production patterns for Production Projects.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Production Projects processing cancelled")
	default:
	}
	if data == "" {

...
**CSV** — file I/O and serialization.

## Exercise 1: Core Implementation

Implement and understand the core **CSV** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./68-csv/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./68-csv/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./68-csv/exercises/solutions/
```
...
```bash
go test ./68-csv/exercises/solutions/ -run TestInterviewChallenge
```

...
// Exercise2Service demonstrates production patterns for Strings, Runes & Bytes.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Strings, Runes & Bytes processing cancelled")
	default:
	}
	if data == "" {

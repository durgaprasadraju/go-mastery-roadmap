...
Implement and understand the core **JSON** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./66-json/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./66-json/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./66-json/exercises/solutions/
```
...
```bash
go test ./66-json/exercises/solutions/ -run TestInterviewChallenge
```

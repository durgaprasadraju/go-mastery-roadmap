...
// Exercise2Service demonstrates production patterns for Divide & Conquer.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Divide & Conquer processing cancelled")
	default:
	}
	if data == "" {

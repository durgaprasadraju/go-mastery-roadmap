...
// Exercise2Service demonstrates production patterns for Design Patterns.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Design Patterns processing cancelled")
	default:
	}
	if data == "" {

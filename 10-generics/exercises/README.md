...
Hands-on exercises for **Generics** — variables, types, idioms, and Go fundamentals.

## Exercise 1: Core Implementation

Implement and understand the core **Generics** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./10-generics/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./10-generics/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./10-generics/exercises/solutions/
```
...
```bash
go test ./10-generics/exercises/solutions/ -run TestInterviewChallenge
```

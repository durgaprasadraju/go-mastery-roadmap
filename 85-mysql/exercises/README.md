...
**MySQL** — SQL, pooling, transactions.

## Exercise 1: Core Implementation

Implement and understand the core **MySQL** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./85-mysql/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./85-mysql/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./85-mysql/exercises/solutions/
```
...
```bash
go test ./85-mysql/exercises/solutions/ -run TestInterviewChallenge
```

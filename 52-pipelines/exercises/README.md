...
**Pipelines** — goroutines, channels, sync primitives.

## Exercise 1: Core Implementation

Implement and understand the core **Pipelines** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./52-pipelines/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./52-pipelines/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./52-pipelines/exercises/solutions/
```
...
```bash
go test ./52-pipelines/exercises/solutions/ -run TestInterviewChallenge
```

# Fast & Slow Pointers — Exercise Solutions

> Attempt exercises in ../README.md before reading these solutions.

## Topic

**Fast & Slow Pointers** — implement, analyze complexity, and benchmark.

## Files

| File | Description |
|------|-------------|
| exercise1.go | Core Fast & Slow Pointers exercise |
| exercise1_test.go | Unit tests |
| exercise2.go | Production patterns (context + structured errors) |
| exercise3_bench_test.go | Performance benchmarks |
| interview-challenge-solution.go | Find duplicate (Floyd) |

## Run

```bash
go test ./35-fast-slow-pointers/exercises/solutions/
go test -bench=. ./35-fast-slow-pointers/exercises/solutions/
```

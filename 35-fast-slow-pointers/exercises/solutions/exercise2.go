...
// Exercise2Service demonstrates production patterns for Fast & Slow Pointers.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Fast & Slow Pointers processing cancelled")
	default:
	}
	if data == "" {

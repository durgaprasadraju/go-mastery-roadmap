...
// Exercise2Service demonstrates production patterns for Segment Trees.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Segment Trees processing cancelled")
	default:
	}
	if data == "" {

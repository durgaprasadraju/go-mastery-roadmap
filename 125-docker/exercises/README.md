...
**Docker** — containers, K8s, CI/CD, IaC.

## Exercise 1: Core Implementation

Implement and understand the core **Docker** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./125-docker/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./125-docker/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./125-docker/exercises/solutions/
```
...
```bash
go test ./125-docker/exercises/solutions/ -run TestInterviewChallenge
```

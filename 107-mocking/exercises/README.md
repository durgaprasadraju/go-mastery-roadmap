...
**Mocking** — unit, integration, fuzz, benchmarks.

## Exercise 1: Core Implementation

Implement and understand the core **Mocking** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./107-mocking/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./107-mocking/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./107-mocking/exercises/solutions/
```
...
```bash
go test ./107-mocking/exercises/solutions/ -run TestInterviewChallenge
```

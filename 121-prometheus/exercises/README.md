...
**Prometheus** — logs, metrics, traces.

## Exercise 1: Core Implementation

Implement and understand the core **Prometheus** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./121-prometheus/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./121-prometheus/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./121-prometheus/exercises/solutions/
```
...
```bash
go test ./121-prometheus/exercises/solutions/ -run TestInterviewChallenge
```

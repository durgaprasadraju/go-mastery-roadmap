...
**Authentication** — auth, crypto, secure coding.

## Exercise 1: Core Implementation

Implement and understand the core **Authentication** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./94-authentication/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./94-authentication/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./94-authentication/exercises/solutions/
```
...
```bash
go test ./94-authentication/exercises/solutions/ -run TestInterviewChallenge
```

...
**Sorting Algorithms** — implement, analyze complexity, and benchmark.

## Exercise 1: Core Implementation

Implement and understand the core **Sorting Algorithms** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./31-sorting/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./31-sorting/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./31-sorting/exercises/solutions/
```
...
```bash
go test ./31-sorting/exercises/solutions/ -run TestInterviewChallenge
```

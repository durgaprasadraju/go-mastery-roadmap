...
// Exercise2Service demonstrates production patterns for String Algorithms.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "String Algorithms processing cancelled")
	default:
	}
	if data == "" {

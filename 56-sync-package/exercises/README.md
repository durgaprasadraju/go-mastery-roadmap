# sync Package — Exercises

**sync Package** — goroutines, channels, sync primitives.

## Exercise 1: Core Implementation

Implement and understand the core **sync Package** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./56-sync-package/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./56-sync-package/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./56-sync-package/exercises/solutions/
```
...
```bash
go test ./56-sync-package/exercises/solutions/ -run TestInterviewChallenge
```

...
// Exercise2Service demonstrates production patterns for Monotonic Queue.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Monotonic Queue processing cancelled")
	default:
	}
	if data == "" {

...
// Exercise2Service demonstrates production patterns for Event Streaming.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Event Streaming processing cancelled")
	default:
	}
	if data == "" {

...
// Exercise2Service demonstrates production patterns for SQL Fundamentals.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "SQL Fundamentals processing cancelled")
	default:
	}
	if data == "" {

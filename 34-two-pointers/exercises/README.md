...
**Two Pointers** — implement, analyze complexity, and benchmark.

## Exercise 1: Core Implementation

Implement and understand the core **Two Pointers** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./34-two-pointers/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./34-two-pointers/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./34-two-pointers/exercises/solutions/
```
...
```bash
go test ./34-two-pointers/exercises/solutions/ -run TestInterviewChallenge
```

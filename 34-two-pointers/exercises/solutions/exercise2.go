...
// Exercise2Service demonstrates production patterns for Two Pointers.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Two Pointers processing cancelled")
	default:
	}
	if data == "" {

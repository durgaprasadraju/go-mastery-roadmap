...
// Exercise2Service demonstrates production patterns for Networking Fundamentals.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Networking Fundamentals processing cancelled")
	default:
	}
	if data == "" {

...
**WebSocket** — protocols, clients, servers.

## Exercise 1: Core Implementation

Implement and understand the core **WebSocket** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./79-websocket/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./79-websocket/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./79-websocket/exercises/solutions/
```
...
```bash
go test ./79-websocket/exercises/solutions/ -run TestInterviewChallenge
```

...
// Exercise2Service demonstrates production patterns for Integration Testing.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Integration Testing processing cancelled")
	default:
	}
	if data == "" {

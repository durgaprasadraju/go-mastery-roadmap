...
**TCP** — protocols, clients, servers.

## Exercise 1: Core Implementation

Implement and understand the core **TCP** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./72-tcp/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./72-tcp/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./72-tcp/exercises/solutions/
```
...
```bash
go test ./72-tcp/exercises/solutions/ -run TestInterviewChallenge
```

...
// Exercise2Service demonstrates production patterns for Worker Pools.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Worker Pools processing cancelled")
	default:
	}
	if data == "" {

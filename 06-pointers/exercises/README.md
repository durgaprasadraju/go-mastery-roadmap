...
Hands-on exercises for **Pointers** — variables, types, idioms, and Go fundamentals.

## Exercise 1: Core Implementation

Implement and understand the core **Pointers** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./06-pointers/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./06-pointers/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./06-pointers/exercises/solutions/
```
...
```bash
go test ./06-pointers/exercises/solutions/ -run TestInterviewChallenge
```

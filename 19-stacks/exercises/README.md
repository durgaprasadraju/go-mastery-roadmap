...
Implement **Stacks** from scratch with tests and complexity analysis.

## Exercise 1: Core Implementation

Implement and understand the core **Stacks** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./19-stacks/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./19-stacks/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./19-stacks/exercises/solutions/
```
...
```bash
go test ./19-stacks/exercises/solutions/ -run TestInterviewChallenge
```

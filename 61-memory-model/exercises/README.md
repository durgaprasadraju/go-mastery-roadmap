# Go Memory Model — Exercises

**Go Memory Model** — goroutines, channels, sync primitives.

## Exercise 1: Core Implementation

Implement and understand the core **Go Memory Model** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./61-memory-model/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./61-memory-model/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./61-memory-model/exercises/solutions/
```
...
```bash
go test ./61-memory-model/exercises/solutions/ -run TestInterviewChallenge
```

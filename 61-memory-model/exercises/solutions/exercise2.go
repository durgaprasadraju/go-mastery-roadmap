...
// Exercise2Service demonstrates production patterns for Go Memory Model.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Go Memory Model processing cancelled")
	default:
	}
	if data == "" {

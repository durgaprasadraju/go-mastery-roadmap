...
// Exercise2Service demonstrates production patterns for Go Type System.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Go Type System processing cancelled")
	default:
	}
	if data == "" {

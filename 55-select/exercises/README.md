...
**Select Statement** — goroutines, channels, sync primitives.

## Exercise 1: Core Implementation

Implement and understand the core **Select Statement** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./55-select/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./55-select/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./55-select/exercises/solutions/
```
...
```bash
go test ./55-select/exercises/solutions/ -run TestInterviewChallenge
```

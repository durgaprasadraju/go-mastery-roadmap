# Server-Sent Events — Exercises

**Server-Sent Events** — protocols, clients, servers.

## Exercise 1: Core Implementation

Implement and understand the core **Server-Sent Events** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./80-server-sent-events/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./80-server-sent-events/exercises/solutions/ -run TestService
```
...
```bash
go test ./80-server-sent-events/exercises/solutions/ -run TestInterviewChallenge
```

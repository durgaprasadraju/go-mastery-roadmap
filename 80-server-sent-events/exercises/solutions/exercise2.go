...
// Exercise2Service demonstrates production patterns for Server-Sent Events.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Server-Sent Events processing cancelled")
	default:
	}
	if data == "" {

...
Implement and understand the core **Context** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./54-context/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./54-context/exercises/solutions/ -run TestService
```
...
**Problem:** Propagate context cancellation

Work in interview-challenge.go for 30 minutes before checking solutions/interview-challenge-solution.go.

```bash
go test ./54-context/exercises/solutions/ -run TestInterviewChallenge
```

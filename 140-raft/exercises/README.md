# Raft Consensus — Exercises

**Raft Consensus** — scalable system components.

## Exercise 1: Core Implementation

Implement and understand the core **Raft Consensus** concept in exercise1.go (try yourself first).

**Run tests:**
```bash
go test ./140-raft/exercises/solutions/ -run TestExercise1
```
...
```bash
go test ./140-raft/exercises/solutions/ -run TestService
```
...
```bash
go test -bench=. ./140-raft/exercises/solutions/
```
...
```bash
go test ./140-raft/exercises/solutions/ -run TestInterviewChallenge
```

...
// Exercise2Service demonstrates production patterns for Raft Consensus.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Raft Consensus processing cancelled")
	default:
	}
	if data == "" {

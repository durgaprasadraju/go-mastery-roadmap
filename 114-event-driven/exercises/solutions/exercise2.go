...
// Exercise2Service demonstrates production patterns for Event-Driven Architecture.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "Event-Driven Architecture processing cancelled")
	default:
	}
	if data == "" {

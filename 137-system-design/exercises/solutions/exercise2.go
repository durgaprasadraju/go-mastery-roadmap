...
// Exercise2Service demonstrates production patterns for System Design.
...
func (s *Exercise2Service) Process(ctx context.Context, data string) (string, error) {
	select {
	case <-ctx.Done():
		return "", apperrors.Wrap(ctx.Err(), apperrors.CodeInternal, "System Design processing cancelled")
	default:
	}
	if data == "" {
